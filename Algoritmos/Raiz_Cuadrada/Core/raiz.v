module raiz_cuadrada (
    ...
    Corrimiento_ra   corr_ra(.clk(clk), .rst(rst), .init(init), .load(w_load_ra), 
                       .radical_in(radical), .shift(w_shift_ra), .sub_en(w_sub_ra), 
                       .C(w_C), .A(w_A), .R(w_R));
                       
    corrimiento_b      corr_b(.clk(clk), .rst(rst), .init(init), .clear(w_clear_b), 
                       .shift(w_shift_b), .set_b0(w_set_b0), .B(w_B));
                       
    Corrimiento_c      corr_c (.clk(clk), .rst(rst), .init(init), .load_b(w_load_c), 
                       .B(w_B), .calc_c(w_calc_c), .C(w_C));
