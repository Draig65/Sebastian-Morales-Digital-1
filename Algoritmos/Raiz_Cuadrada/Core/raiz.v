...
input        clk, rst, start;
    ...
    Corr_b      corr_b(.clk(clk), .start(start),.shift(w_shift_b), .b0(w_set_b0), .B(w_B));                
    ...
    control  ctrl(.clk(clk), .rst(rst), .start(start), .z(w_z), .z_a(w_z_a), .load_ra(w_load_ra), .shift_ra(w_shift_ra), .sub_ra(w_sub_ra), 
                     .shift_b(w_shift_b), .set_b0(w_set_b0), .load_c(w_load_c), .calc_c(w_calc_c), .done(done));
endmodule
